...
% Params
load([folder, 'params_res0.09.mat'], 'params');
...
files = dir([folder, 'predictions_res*.mat']);
merged = [];
...
[~, methods, resolution] = findgroups(predictions.method, predictions.resolution);
id_cat = arrayfun(@(x) [char(methods(x)) ' ' num2str(resolution(x))], 1:length(methods), 'UniformOutput', false);
id_cat = categorical(id_cat);

%% Median error plots
% Compute spatial median errors
% TODO requires that all combinations are present in predictions!
[xy_bins, method, resolution, x, y] = findgroups(...
    predictions.method,...
...
spatial_error = table(method, resolution, x, y, median_error);

% Compute orientational median errors
% filter = predictions.source(:,1) > -0.2 & predictions.source(:,1) < 0.2;
filter = 1:size(predictions.source, 1);
[ay_bins, method, resolution, azi, y] = findgroups(...
    predictions.method(filter,:),...
    predictions.resolution(filter,:),...
...
orientational_error = table(method, resolution, azi, y, median_error);
...
% heatmap_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_error_spatial_heatmap_res' num2str(res) '.jpg'];
% conf.caxis = [0 median_error_thresholds(end-1)];
% conf.colormap = flip(map);
% conf.cell_size = cell_size;
% 
% [ids, methods, resolutions] = findgroups(spatial_error.method, spatial_error.resolution);
% u_id = unique(ids);
% for id = u_id'
%    filter = ids == id; 
%    method = methods(id);
%    resolution = resolutions(id);
%    conf.name = heatmap_name(char(method), resolution);
%    plot_spatial_heatmap(x, y, spatial_error.median_error(filter), params, conf);
% end
% 
% % orientation heatmap plot
% orientation_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_error_orientation_heatmap_res' num2str(res) '.jpg'];
% conf.caxis = [0 median_error_thresholds(end - 1)];
...
% [ids, methods, resolutions] = findgroups(orientational_error.method, orientational_error.resolution);
% u_id = unique(ids);
% for id = u_id'
%    filter = ids == id; 
%    method = methods(id);
%    resolution = resolutions(id);
%    conf.name = orientation_name(char(method), resolution);
...
% drawnow
% pause(0.2)
% print('./images/exp1_train_data/error_heatmap_colorbar.jpg', '-djpeg', '-r600')
% disp('error_heatmap_colorbar')
% close

% spatial contour plot
contour_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_error_spatial_contour_res' num2str(res) '.jpg'];
...
for id = u_id'
   ...
   method = methods(id);
   resolution = resolutions(id);
   conf.name = contour_name(char(method), resolution);
   plot_spatial_contour(x, y, spatial_error.median_error(filter), params, conf);
end
% orientation contour plot
orientation_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_error_orientation_contour_res' num2str(res) '.jpg'];
conf.caxis = [0 median_error_thresholds(end - 1)];
conf.colormaps = flip(map);
conf.cell_size = conf.cell_size;
conf.spokes = 5;
...
drawnow
pause(0.2)
print('./images/exp1_train_data/error_contour_colorbar.jpg', '-djpeg', '-r600')
...
drawnow
pause(0.2)
print('./images/exp1_train_data/error_bar_colorbar.jpg', '-djpeg', '-r600')
disp('error_bar_colorbar')
close

% 
% % spatial block plot
% block_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_error_spatial_block_res' num2str(res) '.jpg'];
% conf.thresholds = median_error_thresholds;
% conf.colormap = map;
% conf.cell_size = cell_size; 
% 
...
%    method = methods(id);
%    resolution = resolutions(id);
%    conf.name = block_name(char(method), resolution);
%    plot_spatial_block(x, y, spatial_error.median_error(filter), params, conf);
% end
% 
% % orientation block plot
% block_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_error_orientation_block_res' num2str(res) '.jpg'];
% conf.thresholds = median_error_thresholds;
% conf.colormap = map;
% conf.cell_size = cell_size; 
% 
...
[ids, method, resolution] = findgroups(spatial_error.method, spatial_error.resolution);
...
threshold_areas = table(method, resolution, area);
...
u_resolutions = unique(threshold_areas.resolution);
n_resolutions = length(u_resolutions);

method_order = {'GN', 'QM', 'NR', 'KNN', 'ELM', 'LCMV', 'CWT', 'RND'};
...
[~, resolution_order] = sort(u_resolutions, 'descend');

area = permute(threshold_areas.area, [1 3 2]);
area = reshape(area, [n_resolutions, n_methods, length(median_error_thresholds)]);
area = area(resolution_order, method_order, :);
area = permute(area, [2 1 3]);
area(:, :, end) = [];

method_labels = arrayfun(@(x) {char(x)}, u_methods(method_order));
resolution_labels = vector_to_labels(u_resolutions(resolution_order) * 100, '', '');

figure
set(groot, 'defaultAxesColorOrder', colors); 
plotBarStackGroups(area, method_labels, resolution_labels);
set(groot, 'defaultAxesColorOrder', d_colors);
ylim([0 max_area])
ylabel('area (m$^2$)')
xlabel('method / resolution ($\times 10^{-2}$)')
post_process_figure(1, 0.45, [1.2 1.4], [0 0.2])

drawnow
pause(0.2)
print('./images/exp1_train_data/median_error_area_bars', '-djpeg', '-r600')
...
% spatial_score = table(method, resolution, x, y, score);
% 
...
% orientational_score = table(method, resolution, azi, y, score);
% 
% x = unique(x);
% y = unique(y);
% a = unique(azi);
% 
% % Spatial heatmap
% heatmap_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_score_spatial_heatmap_res' num2str(res) '.jpg'];
% conf.caxis = [0 1];
% conf.colormap = flip(map);
% conf.cell_size = cell_size;
% 
% [ids, methods, resolutions] = findgroups(spatial_score.method, spatial_score.resolution);
% u_id = unique(ids);
% for id = u_id'
%    filter = ids == id; 
%    method = methods(id);
...
% 
% % orientation  heatmap
% orientation_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_score_orientation_heatmap_res' num2str(res) '.jpg'];
% conf.caxis = [0 1];
% conf.colormaps = flip(map);
% conf.cell_size = cell_size;
% conf.spokes = 5;
% conf.circles = 3;
% 
% [ids, methods, resolutions] = findgroups(orientational_score.method, orientational_score.resolution);
% u_id = unique(ids);
% for id = u_id'
%    filter = ids == id; 
%    method = methods(id);
...
% drawnow
% pause(0.2)
% print('./images/exp1_train_data/score_heatmap_colorbar.jpg', '-djpeg', '-r600')
% disp('score_heatmap_colorbar')
% close
% 
% % spatial contour
% contour_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_score_spatial_contour_res' num2str(res) '.jpg'];
...
% 
% % orientation contour
% orientation_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_score_orientation_contour_res' num2str(res) '.jpg'];
...
% drawnow
% pause(0.2)
% print('./images/exp1_train_data/score_contour_colorbar.jpg', '-djpeg', '-r600')
% disp('error_contour_colorbar')
% close
% 
% % Area score-thresholds
% thresh = error_score_thresholds;
% [ids, method, resolution] = findgroups(spatial_score.method, spatial_score.resolution);
...
% n_resolutions = length(u_resolutions);
% 
% method_order = 1:n_methods; %[3 4 6 5 2 1 7];
% [~, resolution_order] = sort(u_resolutions, 'descend');
% 
% area = permute(threshold_areas.area, [1 3 2]);
% area(:, :, 1) = area(:, :, 2) + area(:, :, 1);
% area(:, :, 2) = [];
% area = reshape(area, [n_resolutions, n_methods, length(error_score_thresholds)-1]);
% area = area(resolution_order, method_order, :);
% area = permute(area, [2 1 3]);
% 
% method_labels = arrayfun(@(x) {char(x)}, u_methods(method_order));
...
% drawnow
% pause(0.2)
% print('./images/exp1_train_data/error_score_area_bars', '-djpeg', '-r600')
% disp('error_score_area_bars')
% close

%% Predicted locations
num_locations = 100;
prediction_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_predictions_res' num2str(res) '.jpg'];
[ids, methods, resolutions] = findgroups(predictions.method, predictions.resolution);
u_id = unique(ids);
max_index = sum(ids == 1);
...
for id = u_id'
    ...
    name = prediction_name(char(method), resolution);
    set(gca, 'tickdir', 'out')
...
spatial_errors = table(method, resolution, x, y, median_location_error, median_orientation_error);
...
xticks(ticks)
xticklabels(vector_to_labels(100*ticks, '', ''))
yticks(ticks)
daspect([1 1 1])
set(gca, 'tickdir', 'out')
...
drawnow
pause(0.2)
name = './images/exp1_train_data/colorcoded_colorbar.jpg';
...
colorcoded_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_colorcoded_spatial_res' num2str(res) '.jpg'];
[ids, methods, resolutions] = findgroups(spatial_errors.method, spatial_errors.resolution);
u_id = unique(ids);
for id = u_id'
    filter = ids == id; 
    method = methods(id);
    resolution = resolutions(id);
    conf.name = colorcoded_name(char(method), resolution);
...
%     figure(2)
%     contourf(u_x, u_y, reshape(L, [length(u_y), length(u_x)]), conf.thresholds)
%     colormap(flip(map))
%     title('location')
%     daspect([1 1 1])
...
%     
%     figure(3)
%     contourf(u_x, u_y, reshape(O, [length(u_y), length(u_x)]), conf.thresholds)
%     colormap(flip(map))
%     title('orientation')
%     daspect([1 1 1])
%     caxis([0 conf.thresholds(end - 1)])
...
orientation_errors = table(method, resolution, azi, y, median_location_error, median_orientation_error);
...
% Make the plots for each method and resolution
colorcoded_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_colorcoded_orientation_res' num2str(res) '.jpg'];
[ids, methods, resolutions] = findgroups(orientation_errors.method, orientation_errors.resolution);
u_id = unique(ids);
for id = u_id'
    filter = ids == id; 
    method = methods(id);
...
%% Plot location and orientation error individually (maximum)
% Spatial medians
location_error = sqrt(sum((predictions.prediction(:, 1:2) - predictions.source(:, 1:2)).^2, 2));
orientation_error = abs(compute_angle_difference(predictions.source(:, 3), predictions.prediction(:, 3)) / pi);
...
[xy_bins, method, resolution, x, y] = findgroups(...
    ...
    predictions.source_bin(:,1),...
    predictions.source_bin(:,2));
max_location_error = splitapply(@max, location_error, xy_bins);
max_orientation_error = splitapply(@max, orientation_error, xy_bins);
spatial_errors = table(method, resolution, x, y, max_location_error, max_orientation_error);
...
% Make the plots for each method and resolution
colorcoded_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_max_colorcoded_spatial_res' num2str(res) '.jpg'];
[ids, methods, resolutions] = findgroups(spatial_errors.method, spatial_errors.resolution);
u_id = unique(ids);
for id = u_id'
    filter = ids == id; 
    ...
    L = spatial_errors.max_location_error(filter);
    O = spatial_errors.max_orientation_error(filter);
...
[ay_bins, method, resolution, azi, y] = findgroups(...
    ...
    predictions.source_bin(filter,3),...
    predictions.source_bin(filter,2));
max_location_error = splitapply(@max, location_error(filter,:), ay_bins);
max_orientation_error = splitapply(@max, orientation_error(filter, :), ay_bins);
orientation_errors = table(method, resolution, azi, y, max_location_error, max_orientation_error);
...
% Make the plots for each method and resolution
colorcoded_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_max_colorcoded_orientation_res' num2str(res) '.jpg'];
[ids, methods, resolutions] = findgroups(orientation_errors.method, orientation_errors.resolution);
u_id = unique(ids);
for id = u_id'
    filter = ids == id; 
    ...
    L = orientation_errors.max_location_error(filter);
    O = orientation_errors.max_orientation_error(filter);
...
%% Plot location and orientation error individually (contour)
% Dont use these, the contour assumes that the error levels follow
% continuously. There is no default order for orientation and location
% errors, so these images do not give a proper view of the data.
% 
% % Spatial medians
% location_error = sqrt(sum((predictions.prediction(:, 1:2) - predictions.source(:, 1:2)).^2, 2));
% orientation_error = abs(compute_angle_difference(predictions.source(:, 3), predictions.prediction(:, 3)) / pi);
...
% conf.orientation_error_min = 0;
% conf.orientation_error_max = median_error_thresholds(end - 1);
% conf.color_low_low = [1 0 1];    % RGB
% conf.color_high_high = [0 0 0];  % RGB
% conf.color_low_high = [1 1 0];   % RGB
% conf.color_high_low = [0 1 1];   % RGB
% 
% % Compute threshold combinations
% [L, O] = meshgrid(median_error_thresholds, median_error_thresholds);
% % Define base colors for interpolation
...
% 
% % Make the plots for each method and resolution
% colorcoded_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_colorcoded_spatial_contour_res' num2str(res) '.jpg'];
% [ids, methods, resolutions] = findgroups(spatial_errors.method, spatial_errors.resolution);
% u_id = unique(ids);
% for id = u_id'
%     filter = ids == id; 
...
%     or_lvls = zeros(size(L, 1), 1);
%     for idx = 1:size(L, 1)
%        loc_lvl = find(L(idx, :) <= median_error_thresholds, 1, 'first');
%        or_lvl =  find(O(idx, :) <= median_error_thresholds, 1, 'first');
%        if isempty(loc_lvl)
%            loc_lvl = 6;
%        end
%        if isempty(or_lvl)
...
%     
%     %clean up indices
%     color_index(color_index == 6) = 30;
%     color_index(color_index == 12) = 30;
%     color_index(color_index == 18) = 30;
%     color_index(color_index == 24) = 30;
%     color_index(color_index > 30) = 30;
%     
%     indices = 1:size(color_comb, 1);
%     indices([6 12 18 24 31:end]) = [];
%     
%     figure
%     [X, Y] = meshgrid(u_x, u_y);
%     contourf(X, Y, color_index, indices)
%     colormap(color_comb)
%     caxis([1 size(color_comb, 1)])
%     set(gca, 'ydir', 'normal')
%     xlabel('$x$ (m)')
...
% 
% % Make the plots for each method and resolution
% colorcoded_name = @(mtd, res) ['./images/exp1_train_data/' lower(mtd) '_colorcoded_orientation_contour_res' num2str(res) '.jpg'];
% [ids, methods, resolutions] = findgroups(orientation_errors.method, orientation_errors.resolution);
% u_id = unique(ids);
% for id = u_id'
%     filter = ids == id; 
...
%     color_index = reshape(color_index, [n_y, n_a]);
%     
%     plot_orientation_colorcoded_contour(u_a, u_y, color_index, params, conf);
% end
...
% 
% distributions = table(...
%     methods, resolution, values(:, 1), values(:, 2),...
