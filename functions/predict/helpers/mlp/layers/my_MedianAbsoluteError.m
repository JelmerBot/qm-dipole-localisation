classdef my_MedianAbsoluteError < nnet.internal.cnn.layer.RegressionLayer
    % my_MedianAbsoluteError   my_MedianAbsoluteError loss output layer
    ...
    methods
        function this = my_MedianAbsoluteError(name)
            % my_MedianAbsoluteError   Constructor for the layer
            this.Name = name;
            this.ResponseNames = {};
        ...
        function loss = forwardLoss( this, Y, T )
            ...
            absoluteError = sum(abs(Y-T), 3);
            N = size(Y, 4);
            loss = median(absoluteError, 4);
        end
    ...
    methods (Static)
        function layer = constructWithObservationDim( name, responseNames, observationDim )
            ...
            layer = nnet.internal.cnn.layer.my_MedianAbsoluteError( name );
