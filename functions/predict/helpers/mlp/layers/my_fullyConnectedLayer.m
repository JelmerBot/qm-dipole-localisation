function layer = my_fullyConnectedLayer( varargin )
% fullyConnectedLayer   Fully connected layer
%
%   layer = my_fullyConnectedLayer(outputSize, initializer) creates a fully connected
...
%                                   ''.
%       'WeightInitializer'       - A function that initializes the weights.
%                                   w = fun(in_size, out_size) where
%                                   size(w) = [out_size in_size]
%
%   Example 1:
...
internalLayer = my_FullyConnected( ...
    args.Name, ...
    args.InputSize, ...
    args.OutputSize);
...
internalLayer.WeightsInitializer = args.WeightsInitializer;
...
defaultName = '';
defaultInitializer = @narrow_normal;
...
p.addParameter('WeightsInitializer', defaultInitializer, @iAssertValidInitializer);
end
...
function iAssertValidInitializer(value)
validateattributes(value, {'function_handle'}, {});
validateattributes(value(2, 6), {'double', 'single'}, ...
    {'nrows' 2, 'ncols', 6, 'real', 'finite'})
end