...
for idx = 1:n_levels
    ...
    errors = compute_prediction_errors(predictions, validate_velocity.sources, params);
    info.test_errors(idx) = median(errors);
    
    predictions = predict_mlp(training_velocity, trained_net, params);
    errors = compute_prediction_errors(predictions, training_velocity.sources, params);
    info.train_errors(idx) = median(errors);
end
