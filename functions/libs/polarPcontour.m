function [varargout] = polarPcontour(R,theta,Z, thresholds,varargin)
% [h,c] = polarPcontour(R,theta,Z,varargin) is a pseudocolor plot of matrix 
...
%% Syntax
% 
% [h,c] = polarPcontour(R,theta,Z)
% [h,c] = polarPcontour(R,theta,Z,'Ncircles',10)
% [h,c] = polarPcontour(R,theta,Z,'Nspokes',5)
% [h,c] = polarPcontour(R,theta,Z,'Nspokes',5,'colBar',0) 
% [h,c] = polarPcontour(R,theta,Z,'Nspokes',5,'labelR','r (km)')
% 
% INPUT
%	* R :
%        - type: float
...
% figure
% polarPcontour(R,theta,Z,'Ncircles',3)
%
%% Author
...
YY = (rNorm)'*cosd(theta);
XX = (rNorm)'*sind(theta);
% e = pcolor(XX,YY,Z,'parent',cax);
h = contourf(XX, YY, Z, thresholds);
if ~isempty(ncolor)
    ...
    function createSpokes(thetaMin,thetaMax,Ncircles,circlesPos,Nspokes)
        ...
        for kk = 1:Nspokes
            ...
            if and(thetaMin==0,thetaMax == 360),
                ...
                v = -(spokeMesh(kk) - 90)/180;
                [n,d] = rat(v);
                str = ['$\frac{' num2str(n) '}{' num2str(d) '}\pi$'];
                if n == 0
                    str = '$0\pi$';
                elseif d == 1
                    str = '$1\pi$';
    ...
    function createCircles(rMin,rMax,thetaMin,thetaMax,Ncircles,circlePos,Nspokes)
        ...
        for kk=1:Ncircles
            ...
            if abs(round(position)) ==90,
                ...
                % radial graduations
               text((contourD(kk)).*cosd(180)+0.05,...
                    -0.15,...
                    rtick,'verticalalignment','BaseLine',...
                    'horizontalAlignment', 'right',...
                    'handlevisibility','off','parent',cax);
